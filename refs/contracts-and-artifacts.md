...
version: 1.0
owner: orchestrator | android-skills | android-cli | understand-anything | graphify
...
```json
{
  ...
  "blocking_gaps": [],
  "non_blocking_gaps": ["architecture_map_missing"],
  "tools": {
...
**Micro mode:** when `workflow_mode = micro`, the context-pack content is written to `session.json["context"]` instead of this file (same sparse rules apply). AUTO-SKIP creating this file; no skip-log entry needed (this is a mode-defined form, not a compliance skip). Any later stage that reads `context-pack.json` must fall back to `session.json["context"]` when this file is absent and `workflow_mode = micro`.
...
Purpose: canonical human-reviewed requirements for the task. This artifact is generated by Orchestrator and must not be treated as accepted until human approval is recorded.
...
### 3) `docs/ai/tasks/{task_id}/design/design-doc.md`
...
- `graph_impact ≥ medium`
- ADR-lite was triggered (`adr_required = true`, including approved or deferred ADR)

Not required when:
- `workflow_mode ∈ {micro, fast}`
- `workflow_mode = standard` and `graph_impact = low` and no ADR trigger fired
...
<chosen approach; link ADR if present; write "No ADR required: <reason>" otherwise>

## Data Model Changes
<Room/schema/preferences/network DTO changes; write "None" when not applicable>

## API Surface Changes
<public/internal APIs, module boundaries, navigation contracts; write "None" when not applicable>

## Testing Strategy
<unit/UI/integration/build/runtime evidence and regression scope>
...
Purpose: final evidence manifest, gate log, task changelog, Artifact Integrity Check, applicable Skill Drift Check, and close status.
...
- Owner is always `orchestrator`; incoming developer adds human notes under "Notes for incoming dev" manually if needed — agent does not overwrite that section after initial write.
...
Required when design-doc trigger is met (`workflow_mode = governed` OR `graph_impact ≥ medium` OR ADR-lite was triggered):
- `docs/ai/tasks/{task_id}/design/design-doc.md` exists and contains the required sections from § 3,

Required when design-doc trigger is not met:
- `implementation-plan.md` exists with ≤ 5 tasks, no placeholders,
- AUTO-SKIP for design doc written to `skip-log.json` with the trigger reason,
...
- Artifact Integrity Check completed, and Skill Drift Check completed when applicable,
...
Before using `android` or `android studio` commands, load `refs/android-cli-compatibility.md` and confirm Stage -1 command discovery/cache says the command is supported. If not, use the fallback matrix from that ref or record an Unavailable-tool record.
...
- `session.json`, `skip-log.json` (lazy — created only on first skip entry), `reports/execution.md`, `evidence/**`
...
```json
{
  ...
  "partial_outputs": [],
  "context": null   // micro mode only: holds sparse context-pack content (same fields as context-pack.json, sparse rules apply); null for non-micro tasks
}
```
...
**Lazy-create:** not pre-created at Stage -1. The file is created on the first write — whichever stage produces the first skip entry. A task that completes with zero skips never writes this file; its absence is not a violation.

Append-only audit log of every skipped step. Written on auto-skips and confirm-skips. Never deleted once created.
...
### Team docs `locks.json`

**Path:** `<TEAM_DOCS_PATH>/active-tasks/locks.json` — shared team-docs conflict index, active only when `TEAM_DOCS_PATH` is configured.
...
**Read:** Stage -1 for conflict detection. **Write:** Stage 0 creates/appends planning locks, Stage 3 upgrades/reconciles final locked files, Stage 7 removes current task locks. If missing at Stage -1, treat as empty; if missing at first write, create it.
...
artifact_budget:
  micro:
    ...
    context_pack: inline               # content inlined into session.json["context"]; no context-pack.json file written
...
| `context-pack.json` | `docs/ai/tasks/{task_id}/clarification/context-pack.json` | Parent orchestrator / Orchestrator | propose raw fields |
| `clarification-brief.md` | `docs/ai/tasks/{task_id}/clarification/clarification-brief.md` | Parent orchestrator / Orchestrator | recommend wording |
| `requirements/<task>.md` | `docs/ai/tasks/{task_id}/requirements/` | Parent orchestrator / Orchestrator | review only |
| `decisions/ADR-*.md` | `docs/ai/decisions/` (GLOBAL) | Parent orchestrator / Orchestrator, Stage 2.5/7 of the active task only | advise; human approves |
| `decisions/README.md` | `docs/ai/decisions/README.md` (GLOBAL) | Parent orchestrator | read only |
| `architecture/*.md` | `docs/ai/architecture/` (GLOBAL) | Parent orchestrator / Orchestrator, Stage 7 of the active task only | domain experts propose section text; human may annotate |
| `architecture/README.md` | `docs/ai/architecture/README.md` (GLOBAL) | Parent orchestrator | read only |
| `design/design-doc.md` | `docs/ai/tasks/{task_id}/design/` | Parent orchestrator / Orchestrator | review only |
| `planning/implementation-plan.md` | `docs/ai/tasks/{task_id}/planning/` | Parent orchestrator / Orchestrator | code owner reads; human may annotate |
| `android-memo/<task>.md` | `docs/ai/tasks/{task_id}/android-memo/` | Android skills | supply advice only |
| `handoff.md` | `docs/ai/tasks/{task_id}/handoff.md` | Parent orchestrator / Orchestrator | human may append notes manually |
...
| Navigation | Orchestrator + Android skills | route/back stack test + ADR-lite if graph changes |
| ViewModel / state | Orchestrator | unit test + state ownership note |
| Repository / data | Orchestrator | unit/integration test + API contract note |
| Database / migration | Android skills + Android CLI | migration test + schema diff |
| Network / API | Orchestrator + Android CLI | contract test + network log/mock evidence |
| DI | Orchestrator | compile/build output + injection path check |
...
| Final go/no-go | Orchestrator | requirements coverage + Gate log |
